...
input_layer* create_input_layer(int size){
    input_layer* input_layer = malloc(sizeof(input_layer));
    input_layer->size = size;
    input_layer->data_vector_input = create_matrix(size,1);
    input_layer->bias = create_matrix(size,1);
    return input_layer;
}
...
void destroy_list_hidden_layer(list_hidden_layer_head* head_of_HL_list){
    ...
    list_hidden_layer_queue* list_hidden_layer_courant = head_of_HL_list->head_of_list;
    list_hidden_layer_queue* list_hidden_layer_next = NULL;
    while(list_hidden_layer_courant != NULL){
        list_hidden_layer_next = list_hidden_layer_courant->next;
        destroy_hidden_layer(list_hidden_layer_courant->hidden_layer_object);
...
void add_hidden_layer_to_list(int number_of_neural, list_hidden_layer_head* list_hidden_layer_tete){
    ...
    if(list_hidden_layer_tete->head_of_list->hidden_layer_object == NULL){
        ...
        list_hidden_layer_tete->head_of_list->hidden_layer_object              = malloc(sizeof(hidden_layer));
        list_hidden_layer_tete->head_of_list->hidden_layer_object->number_hidden_neural      = number_of_neural;
        list_hidden_layer_tete->head_of_list->hidden_layer_object->matrix_weight = malloc(sizeof(matrix));
        list_hidden_layer_tete->head_of_list->hidden_layer_object->bias          = create_matrix(number_of_neural,1);
        list_hidden_layer_tete->head_of_list->hidden_layer_object->function_type = 0;
        list_hidden_layer_tete->head_of_list->hidden_layer_object->vector_data = create_matrix(number_of_neural,1);
        return;
    }
    else{
